...
module tb_floo_nw_mesh;
  ...
  initial begin
    ...
    $display ("[tb_floo_nw_mesh][%0t] Simulation start.", $time);
    // Init signals
    ...
    $display ("[tb_floo_nw_mesh][%0t] Trigger DMA transfers start.", $time);
    start_of_sim = '1;
    repeat (1) @(posedge clk);
    start_of_sim = '0;
    // Wait for all DMA transfers to terminate
    $display ("[tb_floo_nw_mesh][%0t] Waiting for DMA transfers to terminate...", $time);
    wait(&end_of_sim);
    $display ("[tb_floo_nw_mesh][%0t] All DMA transfers have terminated!", $time);
    // Wait for some time
    repeat (2) @(posedge clk);
    // Stop the simulation
    $display ("[tb_floo_nw_mesh][%0t] Simulation end.", $time);
    $stop;
  end
