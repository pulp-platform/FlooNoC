...
<% is_external = ni.endpoint.is_external %>\
...
% if ni.mgr_narrow_port is not None:
  .axi_narrow_in_req_i  ( ${ni.mgr_narrow_port.req_name(port=is_external, idx=is_external)} ),
  .axi_narrow_in_rsp_o  ( ${ni.mgr_narrow_port.rsp_name(port=is_external, idx=is_external)} ),
% else:
  .axi_narrow_in_req_i  ( '0 ),
  .axi_narrow_in_rsp_o  (    ),
% endif
% if ni.sbr_narrow_port is not None:
  .axi_narrow_out_req_o ( ${ni.sbr_narrow_port.req_name(port=is_external, idx=is_external)} ),
  .axi_narrow_out_rsp_i ( ${ni.sbr_narrow_port.rsp_name(port=is_external, idx=is_external)} ),
% else:
  .axi_narrow_out_req_o (    ),
  .axi_narrow_out_rsp_i ( '0 ),
% endif
% if ni.mgr_wide_port is not None:
  .axi_wide_in_req_i  ( ${ni.mgr_wide_port.req_name(port=is_external, idx=is_external)} ),
  .axi_wide_in_rsp_o  ( ${ni.mgr_wide_port.rsp_name(port=is_external, idx=is_external)} ),
% else:
  .axi_wide_in_req_i  ( '0 ),
  .axi_wide_in_rsp_o  (    ),
% endif
% if ni.sbr_wide_port is not None:
  .axi_wide_out_req_o ( ${ni.sbr_wide_port.req_name(port=is_external, idx=is_external)} ),
  .axi_wide_out_rsp_i ( ${ni.sbr_wide_port.rsp_name(port=is_external, idx=is_external)} ),
% else:
  .axi_wide_out_req_o (    ),
