...
def visualize_traffic_gif():
    ...
    filename_list = [
        os.path.join(args.out_dir, "temp" + str(n) + ".png") for n in range(NUM_FRAMES)
    ]
    # get the jobs
    ...
    for frame in range(NUM_FRAMES):
        ...
        for job in jobs:
            ...
            if job[0][0] == job[1][0] and job[0][1] == job[1][1]:
                continue
            coord = (
                job[1][0] * factor + job[0][0] * (1 - factor),
    ...
    command_list = (
        ["convert", "-delay", frame_delay, "-loop", "0"] + filename_list + [output_gif_filename]
    )
...
def visualize_traffic_image():
    ...
    for job in jobs:
        ...
        mid_point = get_position(
            job[0][0] + third_point_distance * math.cos(line_angle + angle),
            job[0][1] + third_point_distance * math.sin(line_angle + angle),
        )
        first_point = (
            get_position(job[0][0], job[0][1])[0] + math.cos(angle + line_angle) * 10,
            get_position(job[0][0], job[0][1])[1] + math.sin(angle + line_angle) * 10,
        )
        last_point = (
            get_position(job[1][0], job[1][1])[0] + math.cos(math.pi - angle + line_angle) * 10,
        ...
        if dist == 1:
            ...
            pygame.gfxdraw.bezier(surf, (first_point, mid_point, last_point), 20, arrow_color)
        ...
        pg.draw.polygon(
            ...
            [
                ...
                (
                    last_point[0]
                    - arrow_tip_length * math.cos(-arrow_tip_angle + line_angle - angle),
                    last_point[1]
                    - arrow_tip_length * math.sin(-arrow_tip_angle + line_angle - angle),
                ),
                (
                    last_point[0]
                    - arrow_tip_length * math.cos(arrow_tip_angle + line_angle - angle),
